#include "aes.h"

const uint8_t AES::sbox[256] = {
    
};

const uint8_t AES::rsbox[256] = {
    
};

AES::AES(const std::vector<uint8_t>& key) {
    key_expansion(key.data());
}

void AES::add_round_key(uint8_t* state, const uint8_t* key) {
    for (int64_t i = 0; i < 16; ++i) state[i] ^= key[i];
}

void AES::sub_bytes(uint8_t* state) {
    for (int64_t i = 0; i < 16; ++i) state[i] = sbox[state[i]];
}

void AES::inv_sub_bytes(uint8_t* state) {
    for (int64_t i = 0; i < 16; ++i) state[i] = rsbox[state[i]];
}

void AES::shift_rows(uint8_t* state) {
    uint8_t tmp[16];
    tmp[0] = state[0]; tmp[1] = state[5]; tmp[2] = state[10]; tmp[3] = state[15];
    tmp[4] = state[4]; tmp[5] = state[9]; tmp[6] = state[14]; tmp[7] = state[3];
    tmp[8] = state[8]; tmp[9] = state[13]; tmp[10] = state[2]; tmp[11] = state[7];
    tmp[12] = state[12]; tmp[13] = state[1]; tmp[14] = state[6]; tmp[15] = state[11];
    for (int64_t i = 0; i < 16; ++i) state[i] = tmp[i];
}

void AES::inv_shift_rows(uint8_t* state) {
    uint8_t tmp[16];
    tmp[0] = state[0]; tmp[1] = state[13]; tmp[2] = state[10]; tmp[3] = state[7];
    tmp[4] = state[4]; tmp[5] = state[1]; tmp[6] = state[14]; tmp[7] = state[11];
    tmp[8] = state[8]; tmp[9] = state[5]; tmp[10] = state[2]; tmp[11] = state[15];
    tmp[12] = state[12]; tmp[13] = state[9]; tmp[14] = state[6]; tmp[15] = state[3];
    for (int64_t i = 0; i < 16; ++i) state[i] = tmp[i];
}

uint8_t xtime(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0x00);
}

void AES::mix_columns(uint8_t* state) {
    for (int64_t i = 0; i < 4; ++i) {
        uint8_t a = state[i * 4];
        uint8_t b = state[i * 4 + 1];
        uint8_t c = state[i * 4 + 2];
        uint8_t d = state[i * 4 + 3];
        state[i * 4] = xtime(a) ^ (xtime(b) ^ b) ^ c ^ d;
        state[i * 4 + 1] = a ^ xtime(b) ^ (xtime(c) ^ c) ^ d;
        state[i * 4 + 2] = a ^ b ^ xtime(c) ^ (xtime(d) ^ d);
        state[i * 4 + 3] = (xtime(a) ^ a) ^ b ^ c ^ xtime(d);
    }
}

void AES::inv_mix_columns(uint8_t* state) {
    for (int64_t i = 0; i < 4; ++i) {
        uint8_t a = state[i * 4], b = state[i * 4 + 1], c = state[i * 4 + 2], d = state[i * 4 + 3];
        auto x = xtime;
        auto x2 = [&](uint8_t v) { return x(x(v)); };
        auto x3 = [&](uint8_t v) { return x(x(x(v))); };
        auto mul = [&](uint8_t v, uint8_t m) {
            uint8_t res = 0;
            if (m == 0x0e) res = x3(v) ^ x2(v) ^ x(v);
            if (m == 0x0b) res = x3(v) ^ x(v) ^ v;
            if (m == 0x0d) res = x3(v) ^ x2(v) ^ v;
            if (m == 0x09) res = x3(v) ^ v;
            return res;
        };
        state[i * 4] = mul(a, 0x0e) ^ mul(b, 0x0b) ^ mul(c, 0x0d) ^ mul(d, 0x09);
        state[i * 4 + 1] = mul(a, 0x09) ^ mul(b, 0x0e) ^ mul(c, 0x0b) ^ mul(d, 0x0d);
        state[i * 4 + 2] = mul(a, 0x0d) ^ mul(b, 0x09) ^ mul(c, 0x0e) ^ mul(d, 0x0b);
        state[i * 4 + 3] = mul(a, 0x0b) ^ mul(b, 0x0d) ^ mul(c, 0x09) ^ mul(d, 0x0e);
    }
}

void AES::key_expansion(const uint8_t* key) {
    for (int64_t i = 0; i < 16; ++i) round_keys[i] = key[i];
    uint8_t rcon = 1;
    for (int64_t i = 16; i < 176; i += 4) {
        uint8_t tmp[4] = {round_keys[i - 4], round_keys[i - 3], round_keys[i - 2], round_keys[i - 1]};
        if (i % 16 == 0) {
            uint8_t t = tmp[0];
            tmp[0] = sbox[tmp[1]] ^ rcon;
            tmp[1] = sbox[tmp[2]];
            tmp[2] = sbox[tmp[3]];
            tmp[3] = sbox[t];
            rcon = xtime(rcon);
        }
        for (int64_t j = 0; j < 4; ++j) round_keys[i + j] = round_keys[i - 16 + j] ^ tmp[j];
    }
}

void AES::encrypt(const uint8_t* input, uint8_t* output) {
    uint8_t state[16];
    for (int64_t i = 0; i < 16; ++i) state[i] = input[i];
    add_round_key(state, round_keys);
    for (int64_t r = 1; r < 10; ++r) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_keys + r * 16);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys + 160);
    for (int64_t i = 0; i < 16; ++i) output[i] = state[i];
}

void AES::decrypt(const uint8_t* input, uint8_t* output) {
    uint8_t state[16];
    for (int64_t i = 0; i < 16; ++i) state[i] = input[i];
    add_round_key(state, round_keys + 160);
    for (int64_t r = 9; r >= 1; --r) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, round_keys + r * 16);
        inv_mix_columns(state);
    }
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, round_keys);
    for (int64_t i = 0; i < 16; ++i) output[i] = state[i];
}
