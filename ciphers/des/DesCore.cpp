...
static uint64_t bytesToBlock(const unsigned char bytes[kBlockSize]) {
    uint64_t block = 0;
    for (int j = 0; j < kBlockSize; ++j) {
        block |= (uint64_t)bytes[j] << (56 - j * 8);
    }
    return block;
}

static void blockToBytes(uint64_t block, unsigned char bytes[kBlockSize]) {
    for (int j = 0; j < kBlockSize; ++j) {
        bytes[j] = (unsigned char)((block >> (56 - j * 8)) & 0xFF);
    }
}
...
static uint64_t encryptBlock64(uint64_t block, const uint64_t subkeys[16]) {
...
static uint64_t decryptBlock64(uint64_t block, const uint64_t subkeys[16]) {
    block = permute(block, IP, 64);
    uint32_t L = (uint32_t)(block >> 32);
    uint32_t R = (uint32_t)block;
...
void encryptBlock(const unsigned char plaintext[kBlockSize], unsigned char ciphertext[kBlockSize], const unsigned char key[kKeySize]) {
    uint64_t subkeys[16];
    generateSubkeys(bytesToBlock(key), subkeys);
    blockToBytes(encryptBlock64(bytesToBlock(plaintext), subkeys), ciphertext);
}
...
void decryptBlock(const unsigned char ciphertext[kBlockSize], unsigned char plaintext[kBlockSize], const unsigned char key[kKeySize]) {
    uint64_t subkeys[16];
    generateSubkeys(bytesToBlock(key), subkeys);
    blockToBytes(decryptBlock64(bytesToBlock(ciphertext), subkeys), plaintext);
}
